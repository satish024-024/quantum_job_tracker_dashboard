...
function rot_phosphor(axis_op, angle, state, divider=10) {
    ...
    var hist = {
        ...
        mode: 'lines',
        opacity: 1.0,
        line: {color: (document.getElementById('phosphor_color') ? document.getElementById('phosphor_color').value : '#1a237e'), width:3},
    }
    PHOSPHOR.push(hist);
}
...
function rabi_plot(data=null) {
    ...
    const pulselengthEl = document.getElementById('pulselength');
    const detuningEl = document.getElementById('detuning');
    const amplitudeEl = document.getElementById('amplitude');
    const rabiDivEl = document.getElementById('rabi_div');

    if (!pulselengthEl || !detuningEl || !amplitudeEl || !rabiDivEl) {
        console.log('📊 Rabi plot elements not found - skipping rabi_plot');
        return;
    }

    console.log('📈 Generating rabi plot...');

    time = pulselengthEl.value;
   if (data === null) {
    t_stop = Math.max(2,time);
    tax = linspace(0,t_stop,101);
    detune = 2*math.PI*detuningEl.value;
    w1 = 2*math.PI*amplitudeEl.value;
