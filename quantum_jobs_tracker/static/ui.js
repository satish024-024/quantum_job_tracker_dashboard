// Check if we're in a dashboard environment where these functions might not be available
if (typeof gen_state === 'function' &&
    ...
    console.log('🎯 Initializing Blochy in standalone mode...');
    ...
    // Only call rabi_plot if rabi_div exists
    if (document.getElementById('rabi_div')) {
        rabi_plot();
    }

} else {
    console.log('📊 Blochy dependencies not available - skipping initialization');
    // Define global variables to prevent undefined errors
    window.QMSTATEVECTOR = [];
    window.BLOCHSPHERE = [];
    window.STATEARROW = [];
    window.PHOSPHOR = [];
