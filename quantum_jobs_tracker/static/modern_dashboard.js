...
class ModernDashboard {
    ...
    async updateResultsWidget() {
        ...
        try {
            ...
            if (data.success && data.results_data) {
                ...
                contentElement.innerHTML = '<p style="text-align: center; color: var(--text-secondary);">No results data available</p>';
            }
        } catch (error) {
    ...
    async updateQuantumStateWidget() {
        ...
        try {
            ...
            console.error('Error loading quantum state data:', error);
    ...
    createWidgetHtml(widgetType) {
        const widgetTitles = {
            ...
            'bloch-sphere': { icon: 'fas fa-globe', title: '3D Bloch Sphere (Blochy)' },
            'circuit': { icon: 'fas fa-cube', title: '3D Quantum Circuit' },
        ...
        const widgetInfo = widgetTitles[widgetType] || { icon: 'fas fa-cube', title: 'Widget' };
            ...
            <div class="widget fade-in" data-widget="${widgetType}">
                ...
                <div class="widget-content">
                    <div class="loading" id="${widgetType}-loading">
                        ...
                        <span>Loading ${widgetInfo.title.toLowerCase()}...</span>
                    </div>
                    <div id="${widgetType}-content" style="display: none;">
                        <!-- ${widgetInfo.title} content will be loaded here -->
                    </div>
                </div>
            </div>
        `;
    ...
    async handleAIQuery() {
        ...
        try {
            if (this.state.aiEnabled && this.aiClient) {
                // Use Google Gemini AI
                const model = this.aiClient.getGenerativeModel({ model: "gemini-pro" });
                const result = await model.generateContent(query);
                const aiResponse = await result.response;
                response.textContent = aiResponse.text();
            } else {
                // Fallback to local AI response
                response.textContent = this.generateAIResponse(query);
            }
        } catch (error) {
